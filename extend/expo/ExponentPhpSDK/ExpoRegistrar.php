...
class ExpoRegistrar
{
    ...
    public function registerInterest($interest, $token)
    {
        if (! $this->isValidExpoPushToken($token)) {
            ajaxError( ExpoRegistrarException::invalidToken() );
        }

        $stored = $this->repository->store($interest, $token);

        if (!$stored) {
           ajaxError( ExpoRegistrarException::couldNotRegisterInterest() );
        }
    ...
    public function removeInterest($interest, $token = null)
    {
        if (!$this->repository->forget($interest, $token)) {
            ajaxError( ExpoRegistrarException::couldNotRemoveInterest() );
        }
     ...
     * @return array
     */
    public function getInterests($interests = array() )
    {
        $tokens = [];
        ...
        if (empty($tokens)) {
            ajaxError( ExpoRegistrarException::emptyInterests() );
        }
