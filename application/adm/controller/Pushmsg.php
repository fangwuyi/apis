...
class Pushmsg extends Base {
    ...
    {
        require_once Env::get('extend_path'). 'umeng/notification/android/AndroidBroadcast.php';
        require_once Env::get('extend_path'). 'umeng/notification/android/AndroidFilecast.php';
        require_once Env::get('extend_path'). 'umeng/notification/android/AndroidGroupcast.php';
        require_once Env::get('extend_path'). 'umeng/notification/android/AndroidUnicast.php';
        require_once Env::get('extend_path'). 'umeng/notification/android/AndroidCustomizedcast.php';

        require_once Env::get('extend_path'). 'umeng/notification/ios/IOSBroadcast.php';
        require_once Env::get('extend_path'). 'umeng/notification/ios/IOSFilecast.php';
        require_once Env::get('extend_path'). 'umeng/notification/ios/IOSGroupcast.php';
        require_once Env::get('extend_path'). 'umeng/notification/ios/IOSUnicast.php';
        require_once Env::get('extend_path'). 'umeng/notification/ios/IOSCustomizedcast.php';

        $this->appkey = '5d3820a0570df3129b000333';
        $this->appMasterSecret = 'jdf1fotxdgsokuhe9zfmrn4glu8ut9bj';
        $this->timestamp = strval(time());
    }

    public function index(){
        $device_tokens = $this->request->post('device_tokens');
        // $device_tokens = 'f9949dfc65a308026b1b89496e80760d813c0a5d062d383e92b14e12c3d15c4a';
        $alert = $this->request->post('alert','IOS 单播推送测试');
        $test = $this->request->post('test','helloworld');
        if( empty($device_tokens) ){
            ajaxReturn(MSG_PARAM_MISS_CODE,'device_tokens 不能为空');
        }
        if( empty($alert) ){
            ajaxReturn(MSG_PARAM_MISS_CODE,'alert 不能为空');
        }
        ...
        try {
            ...
            $unicast->setCustomizedField("test", $test);
            $pageData['rst'] = $unicast->send();
            ajaxReturn(MSG_SUCCESS_CODE,'ok',$pageData);
        } catch (Exception $e) {
            $pageData  = $e->getMessage();
            ajaxReturn(MSG_QUERY_FAIL_CODE,'ok',$pageData);
        }
    }
